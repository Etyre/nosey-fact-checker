...
final class HotKeyManager {
    ...
    /// Registers a combo like "ctrl+alt+cmd+f". Returns nil on success or an error description.
    @discardableResult
    func register(_ combo: String) -> String? {
        unregister()
        guard let (code, mods) = HotKeyManager.parse(combo) else {
            return "Could not parse “\(combo)”. Use e.g. ctrl+alt+cmd+f or shift+cmd+space."
        }
        guard mods != 0 else { return "Add at least one modifier (ctrl, alt, cmd, shift)." }
        let id = EventHotKeyID(signature: OSType(0x4E534559), id: 1) // 'NSEY'
